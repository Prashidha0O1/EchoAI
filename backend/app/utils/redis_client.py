...
class RedisClient:
    """Redis client singleton for rate limiting and token storage"""
...
def increment_login_attempts(email: str) -> tuple[bool, int]:
    ...
    if current_attempts >= settings.MAX_LOGIN_ATTEMPTS:
        # Set lockout
        client.setex(lockout_key, settings.LOGIN_LOCKOUT_SECONDS, "locked")
...
def store_password_reset_token(email: str, token: str) -> None:
    ...
    client.setex(key, settings.PASSWORD_RESET_TOKEN_EXPIRE, email)
